...
function initBasePath() {
    ...
    if (pathname.startsWith('/profreelance/') || pathname === '/profreelance') {
        ...
        BASE_PATH = '/portfolio';
    } else if (pathname.startsWith('/lol/') || pathname === '/lol') {
        BASE_PATH = '/lol';
    } else {
        BASE_PATH = '';
    }
...
function saveCartData(cartData) { cart = cartData; saveCartToStorage(); }
...
function renderSummaryForCart() {
    ...
    let discountPercent = 0, discountAmount = 0;
    ...
    let summaryHtml = `<div class="summary-line summary-subtotal"><span>Товары (${cart.reduce((sum, item) => sum + (item.quantity || 1), 0)} шт.)</span><span>${formatPrice(subtotal)}</span></div>`;
    if (discountAmount > 0) {
        summaryHtml += `<div class="summary-line summary-discount"><span>Скидка (${discountPercent}%)</span><span class="discount-amount">-${formatPrice(discountAmount)}</span></div>`;
    }
    summaryHtml += `<div class="summary-line summary-fee"><span>Комиссия платформы (5%)</span><span>${formatPrice(fee)}</span></div>`;
...
function renderCartPage() {
    ...
    cart.forEach(item => {
        ...
        cartHtml += `<div class="cart-item" data-id="${item.id}"><div class="item-avatar" style="background:linear-gradient(135deg,#1a5ce5,#3593F2);">${escapeHtml(avatarLetter)}</div><div class="item-info"><div class="item-title">${escapeHtml(itemName)}</div><div class="item-author"><i class="fas fa-user"></i>${escapeHtml(itemAuthor)}</div><div class="item-params"><span class="item-param"><i class="fas fa-clock"></i>${service && service.deadline ? service.deadline + ' дн.' : '—'}</span></div></div><div class="item-right"><div class="item-price">${formatPrice(totalPrice)}</div><div class="cart-quantity-controls"><button class="qty-btn qty-minus" data-id="${item.id}"><i class="fas fa-minus"></i></button><span class="qty-value">${quantity}</span><button class="qty-btn qty-plus" data-id="${item.id}"><i class="fas fa-plus"></i></button><button class="item-delete" data-id="${item.id}"><i class="fas fa-trash-alt"></i></button></div></div></div>`;
    });
    cartHtml += '</div>';
...
function renderRecommendationsForCart() {
    ...
    recommended.forEach(service => {
        html += `<div class="rec-card" data-id="${service.id}"><div class="rec-info"><div class="rec-title">${escapeHtml(service.name)}</div><div class="rec-author">${escapeHtml(service.author)} ★★★★☆</div><div class="rec-price">от ${formatPrice(service.price)}</div></div><button class="rec-btn add-to-cart-rec" data-id="${service.id}" data-name="${escapeHtml(service.name)}" data-price="${service.price}" data-author="${escapeHtml(service.author)}">В корзину</button></div>`;
    });
...
function renderServices() {
    ...
    if (toShow.length === 0) {
        grid.innerHTML = `<div style="text-align:center; padding: 60px 20px; color: var(--text-gray);"><i class="fas fa-search" style="font-size: 3rem; margin-bottom: 16px; opacity: 0.5;"></i><p style="font-size: 1rem; font-weight: 600;">Ничего не найдено</p><p style="font-size: 0.85rem; margin-top: 8px;">Попробуйте изменить параметры поиска или фильтры</p></div>`;
        const loadBtn = document.getElementById('loadMoreBtn');
        if (loadBtn) loadBtn.style.display = 'none';
        return;
    }
    grid.innerHTML = toShow.map(s => `<div class="service-card" onclick="location.href='${getCorrectPath('/assets/pages/usluga.html?id=' + s.id)}'"><div class="card-author"><div class="avatar">${s.authorAvatar}</div><div><div class="card-author-name">${s.author}</div><div class="card-rating"><i class="fas fa-star"></i> ${s.rating}</div></div></div><div class="card-title">${s.name}</div><div class="tags">${s.tags.map(t => `<span class="tag">${t}</span>`).join('')}</div><div class="card-footer"><div class="card-price">от ${s.price.toLocaleString('ru-RU')} ₽</div><button class="btn-add-cart-small" onclick="event.stopPropagation(); addToCart({id:${s.id}, name:'${s.name.replace(/'/g, "\\'")}', price:${s.price}, author:'${s.author}'})"><i class="fas fa-cart-plus"></i> В корзину</button></div></div>`).join('');
    const loadBtn = document.getElementById('loadMoreBtn');
    if (loadBtn) {
    ...
    if (search) {
        search.addEventListener('input', (e) => {
            ...
            if (!q) {
                ...
                currentServices = servicesData.filter(s => s.name.toLowerCase().includes(q) || s.author.toLowerCase().includes(q) || s.tags.some(t => t.toLowerCase().includes(q)));
                visibleCount = 6;
                renderServices();
            }
        });
...
function updateOrdersTable(orders) {
    ...
    tbody.innerHTML = orders.map(order => `<tr><td class="order-num">#${order.id}</td><td><div class="order-service-name">${escapeHtml(order.service)}</div><div class="order-exec"><i class="fas fa-user"></i> ${escapeHtml(order.executor)}</div></td><td>${order.date}</td><td><span class="status-badge ${order.status === 'in_work' ? 'status-in-work' : 'status-done'}"><i class="fas ${order.status === 'in_work' ? 'fa-circle' : 'fa-check-circle'}"></i> ${order.status === 'in_work' ? 'В работе' : 'Выполнен'}</span></td><td>${order.amount.toLocaleString()} ₽</td><td><div class="action-btns"><button class="btn-sm btn-sm-primary" onclick="viewOrder(${order.id})">${order.status === 'in_work' ? 'Открыть' : 'Просмотр'}</button></div></td></tr>`).join('');
}
...
function updateFavoritesList(favorites) {
    ...
    favoritesList.innerHTML = favorites.map(fav => `<div class="favorite-item"><div class="favorite-avatar" style="background:linear-gradient(135deg,#1a5ce5,#3593F2);">${fav.avatar}</div><div class="favorite-info"><div class="favorite-title">${escapeHtml(fav.title)}</div><div class="favorite-meta">${escapeHtml(fav.author)} · от ${fav.price.toLocaleString()} ₽</div></div><button class="btn-open" onclick="window.location.href='${getCorrectPath('/assets/pages/usluga.html?id=' + fav.id)}'">Открыть</button></div>`).join('');
}
...
function getFilteredUsers() {
    ...
    if (currentUserSearch) {
        filtered = filtered.filter(u => u.name.toLowerCase().includes(currentUserSearch) || u.email.toLowerCase().includes(currentUserSearch) || (u.phone && u.phone.includes(currentUserSearch)));
    }
    switch (currentUserFilter) {
        ...
        case 'freelancer': filtered = filtered.filter(u => u.type === 'freelancer'); break;
        case 'client': filtered = filtered.filter(u => u.type === 'client'); break;
        default: break;
    }
    return filtered;
}
...
function renderUsersTable() {
    ...
    tbody.innerHTML = paginatedUsers.map(user => `<tr data-user-id="${user.id}"><td><div class="user-cell"><div class="user-av" style="background:linear-gradient(135deg,#1a5ce5,#3593F2);">${user.avatar}</div><div><div class="user-name">${escapeHtml(user.name)}</div><div class="user-email">${escapeHtml(user.email)}</div></div></div></td><td><span class="type-badge ${user.type === 'freelancer' ? 'type-freelancer' : 'type-client'}">${user.type === 'freelancer' ? 'Фрилансер' : 'Заказчик'}</span></td><td><span class="${user.status === 'active' ? 'status-badge-active' : 'status-badge-blocked'}">${user.status === 'active' ? 'Активный' : 'Заблокирован'}</span></td><td>${user.phone || '—'}</td><td>${user.date}</td><td><div class="action-buttons"><button class="btn-icon-sm edit" onclick="editUser(${user.id})" title="Редактировать"><i class="fas fa-edit"></i></button>${user.status === 'active' ? '<button class="btn-icon-sm block" onclick="toggleBlockUser(' + user.id + ')" title="Заблокировать"><i class="fas fa-ban"></i></button>' : '<button class="btn-icon-sm edit" onclick="toggleBlockUser(' + user.id + ')" title="Разблокировать" style="color:var(--success);"><i class="fas fa-check-circle"></i></button>'}<button class="btn-icon-sm delete" onclick="deleteUser(${user.id})" title="Удалить"><i class="fas fa-trash-alt"></i></button></div></td></tr>`).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (headerLoginBtn && headerLogoutBtn) {
        if (user) {
            ...
            headerLogoutBtn.onclick = window.logoutUser;
        } else {
            headerLoginBtn.style.display = 'block';
            headerLogoutBtn.style.display = 'none';
            headerLoginBtn.onclick = () => window.openLoginModal();
        }
    }
    ...
    if (mobileHeaderLoginBtn && mobileHeaderLogoutBtn) {
        if (user) {
            ...
            mobileHeaderLoginBtn.onclick = () => window.openLoginModal();
        }
    }
    ...
    if (switchToRegisterBtn) switchToRegisterBtn.onclick = (e) => { e.preventDefault(); window.switchToRegister(); };
    const switchToLoginBtn = document.getElementById('switchToLoginBtn');
    if (switchToLoginBtn) switchToLoginBtn.onclick = (e) => { e.preventDefault(); window.switchToLogin(); };
    ...
    if (enable2FABtn) enable2FABtn.onclick = () => showNotification('Функция будет доступна soon');
